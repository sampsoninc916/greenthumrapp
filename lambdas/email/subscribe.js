...
export const handler = async (event = {}) => {
  ...
  const logger = createLogger({ requestId, handler: "emailSubscribe" });
  ...
  try {
    ...
    logger.info("Email subscribed", { email: body.email });

    return ok({ status: "subscribed" });
  } catch (error) {
    return handleError(error, { handler: "emailSubscribe", requestId });
  }
};
